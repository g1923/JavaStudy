...
public class Practice3 {
    ...
    // 알파벳 크기만큼의 배열을 만든다
    // char[] cnt = new char['z'-'a' + 1];
    // 반복문을 통해 s1의 알파벳이 존재하면 cnt[] 에 해당하는 알파벳을 1로 바꾼다.
    // 반복문을 통해 s2의 알파벳이 존재한다면 cnt[] 에 해당하는 알파벳을 -1 한다.
    // 조건) 인접한 문자열을 기준으로 값을 비교한다.
    // s1 = [a,b]  s2 = [b,c,a,b]
    // cnt = [1,1]
    // 1) b,c 만 비교했을 경우 => cnt = [1,0,-1]
    // 2) c,a                => cnt = [0,1,-1] : b는인접하지 않은 구간이여서 빼줬던 값을 다시 올려줘야함.
    // 3) a,b                => cnt = [-1,0,0] : c **
